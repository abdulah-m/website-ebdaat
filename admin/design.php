...
if(isset($_GET["q"])){
    ...
    <table class="ads">  
      <tbody>
            <?php
            $select = "SELECT * FROM design WHERE id=$id_show";
            ...
            $select = "SELECT * FROM design_type WHERE id=$id_type";
            $query  = mysql_query($select)or die (mysql_error());
            ...
            echo '  <tr>
                    ...
                    <tr>
                        <th>معاينة</th>
                        <td><img width="100%" src="../images/Design/full/'.$image.'"/></td>
                    </tr>
                    <tr>
                        <td colspan="2">
...
<?php 
        ...
        if(isset($_POST['update'])){
            ...
            $query = "UPDATE design SET id_type='$type',title='$title',description='$description',number='$number',price='$price' ,title_en='$title_en',description_en='$description_en',title_tr='$title_tr',description_tr='$description_tr' WHERE id=$id_edit";
    ...
    <table class="ads">  
      <tbody>
          <form method="POST" action="">
            <?php
            $select = "SELECT * FROM design WHERE id=$id_edit";
            $query  = mysql_query($select)or die (mysql_error());
            $row    = mysql_fetch_array($query);
            $id             =   htmlspecialchars($row['id']);
            $title          =   htmlspecialchars($row['title']);
            ...
            echo '  <tr>
                    ...
                    <tr>
                        ...
                        $q  = mysql_query("SELECT * FROM design_type")or die (mysql_error());
...
<?php
        ...
        $r = mysql_fetch_row(mysql_query("SELECT * FROM design WHERE id=$id_del")) or die (mysql_error());
        $pathName = htmlspecialchars($r[5]);
        
        $sql = "DELETE FROM design WHERE id=$id_del";
        ...
        if(! $retval )
        {
            ...
            if(file_exists($pathF)){
                unlink("../images/Design/full/".$pathName);
                unlink("../images/Design/recent/".$pathName);
        ...
        if(isset($_POST['submit'])){
            ...
            if(!empty($type) && !empty($title) && !empty($number) && !empty($price)){
                ...
                if(!empty($_FILES["file"])){
                    $sel = "SELECT * FROM design";
                    $q   = mysql_query($sel) or die (mysql_error());
                    $c   = mysql_num_rows($q)+1;
                    
                    $temp = explode(".", $_FILES["file"]["name"]);
                    $extension = end($temp);
                    if (move_uploaded_file($_FILES["file"]["tmp_name"],"../images/Design/full/item0".$c.".".$extension)) {
                        $target_path = "item0".$c.".".$extension;
                        smart_resize_image("../images/Design/full/".$target_path,null,600,455);
                        
                        copy("../images/Design/full/".$target_path,"../images/Design/recent/".$target_path);
                        smart_resize_image("../images/Design/recent/".$target_path,null,290,220);
                    }
                    else {
                        $target_path = "no-image.jpg";
                    }
                ...
                $sql ="INSERT INTO design (`id_type`, `thumb`, `title`,`description`, `image`, `number`, `price`, `title_en`, `description_en`, `title_tr`, `description_tr`) VALUES ('$type','$target_path','$title_ar','$description_ar','$target_path','$number','$price','$title_en','$description_en','$title_tr','$description_tr')";
                $retval = mysql_query($sql);
                if(! $retval ){
            ...
            <input class="form-control required" type="text" name="title_ar" placeholder=" عنوان عربي" required>
            <input class="form-control required" type="text" name="title_en" placeholder=" عنوان انكليزي" required>
            <input class="form-control required" type="text" name="title_tr" placeholder=" عنوان تركي" required><br />
            ...
            <textarea class="form-control required" name="description_ar" placeholder="وصف عربي"></textarea>
            <textarea class="form-control required" name="description_en" placeholder="وصف انكليزي"></textarea>
            <textarea class="form-control required" name="description_tr" placeholder="وصف تركي"></textarea><br />
...
<table class="ads">
  ...
  <tbody>
      <?php
        $select = "SELECT * FROM design ORDER BY id DESC";
        $query  = mysql_query($select)or die (mysql_error());
        while($row=mysql_fetch_array($query)){
            ...
            echo '<tr>
                    ...
                    <td>'.$number.'</td>
                    <td>'.$price.'</td>
                    <td><img width="100px" src="../images/Design/recent/'.$thumb.'" /></td>
                    <td colspan="2">
